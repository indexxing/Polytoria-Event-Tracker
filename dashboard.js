...
let ItemInfoCache;
chrome.storage.local.get('INDEX-EGG-HUNT-24-ITEM-INFO', function(result) {
...
function LoadItems() {
    ...
    ItemIDs.forEach(async (id, index) => {
        ...
        if (ItemInfoCache.items[id] === undefined) {
            ...
            ItemInfo = await fetch('https://api.polytoria.com/v1/store/' + id)
            ItemInfo = await ItemInfo.json()
        } else {
            console.log('Using cache', id)
            ItemInfo = await ItemInfoCache.items[id]
        ...
        Elements.ItemRow.appendChild(ItemCard)
    ...
    console.log('done loading', [...Elements.ItemRow.children], Elements.ItemRow.children)
    /*
    const SortedResult = Array.from(Elements.ItemRow.children).sort((a, b) => {
        const aItemID = parseInt(a.getAttribute('href').split('/')[2]);
        const bItemID = parseInt(b.getAttribute('href').split('/')[2]);
        console.log(aItemID, bItemID)
        return (aItemID < bItemID) ? -1 : (aItemID > bItemID) ? 1 : 0;
    });
    const SortedResult = [...Elements.ItemRow.children].sort((a, b) => a.getAttribute('href').split('/')[2] - b.getAttribute('href').split('/')[2])
    
    console.log(SortedResult)
    Elements.ItemRow.innerHTML = SortedResult;
}
*/

/*
    This function was updated from it's original code with the use of AI
    The code has still be reviewed and updated to fit my code style
*/
function LoadItems() {
    Elements.ItemRow.innerHTML = ``;
